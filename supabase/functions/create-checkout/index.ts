...
serve(async (req) => {
  ...
  // Create a Supabase client for reading user data
  ...
  try {
    ...
    if (!plan) throw new Error("Missing plan parameter");
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `${req.headers.get("origin")}/pricing?subscription_canceled=true`,
      subscription_data: {
        metadata: {
          user_id: user.id,
          plan_type: planName,
