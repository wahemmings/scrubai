  ...
  Deno.serve(async (req) => {
    ...
    try {
      ...
      try {
        ...
        // CRITICAL: When building the response, we need to use the EXACT SAME parameter names
        // that will be used in the FormData during upload
        const responseData: Record<string, string | number> = {
          signature,
          timestamp,
          cloudName,
          api_key: apiKey, // IMPORTANT: Cloudinary expects 'api_key', not 'apiKey'
          folder,
          upload_preset: uploadPreset // IMPORTANT: Cloudinary expects 'upload_preset', not 'uploadPreset'
        };
        
        // If public_id was used in signature generation, it MUST be included in the response
        // with the EXACT same parameter name
        if (public_id) {
          // Use exactly 'public_id' as this is what Cloudinary expects
          responseData.public_id = public_id;
          
          // For backward compatibility with existing code that expects publicId
          responseData.publicId = public_id;
          
          console.log("Including public_id in response:", public_id);
        }
